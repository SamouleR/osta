    ...
    renderSlide0Charts(data) {
        ...
        const drawGauge = (containerId, val, color, label, emptyColor = 'rgba(128,128,128,0.2)') => {
            ...
            if (Array.isArray(val)) {
                 pieData = pie([
                     {value: val[0], color: color[0]},
                     {value: val[1], color: color[1]}
                 ]);
            } else {
                 pieData = pie([
                     {value: val, color: color},
                     {value: 100 - val, color: emptyColor}
                 ]);
            }
            
            const arc = d3.arc().innerRadius(radius * 0.65).outerRadius(radius).cornerRadius(5);
            ...
            if (!Array.isArray(val)) {
                svg.append("text").attr("text-anchor", "middle").attr("dy", "0em")
                    ...
                    .text(`${val}%`);
                svg.append("text").attr("text-anchor", "middle").attr("dy", "2.2em")
                    .style("font-size", "1.3rem").style("font-weight", "800").style("fill", "var(--text-main)").style("text-transform", "uppercase").style("letter-spacing", "3px")
                    .text(label);
            } else {
                svg.append("text").attr("text-anchor", "middle").attr("dy", "-0.6em")
                    .style("font-size", "2.8rem").style("font-weight", "900").style("fill", color[0])
                    .text(`${val[0]}% H`);
                svg.append("text").attr("text-anchor", "middle").attr("dy", "1.8em")
                    .style("font-size", "2.8rem").style("font-weight", "900").style("fill", color[1])
                    .text(`${val[1]}% F`);
            }
        };
